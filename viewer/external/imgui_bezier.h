...
namespace ImGui
{
  ...
  int Bezier( const char *label, float P[4] ) {
    ...
    //hovered |= 0 != IsHovered(ImRect(bb.Min, ImVec2(bb.Min.x + avail, bb.Min.y + dim)), id);
    hovered |= 0 != ItemHoverable(ImRect(bb.Min, ImVec2(bb.Min.x + avail, bb.Min.y + dim)), id);
      ...
      if(hovered||changed) DrawList->PushClipRectFullScreen();
        ...
        // draw curve
        {
          ...
          for( int i = 0; i < SMOOTHNESS; ++i ) {
            ImVec2 p = ImVec2(results[i+0].x, 1 - results[i+0].y);
            ImVec2 q = ImVec2(results[i+1].x, 1 - results[i+1].y);
